...
- **项目类型**:大学生创新创业训练计划（大创）省级立项实践项目
- **痛点**:高校编程课师生比悬殊（普遍 1:80+），学生课后答疑排队长、反馈延迟；教师难以掌握每个学生的知识盲点
- **目标**:用 AI 助教 7×24 接管重复性答疑，让教师聚焦于教学设计；通过学习数据看板实现**因材施教**的精细化教学
- **特色**:与市面通用 AI 答题工具不同，本系统把 AI 能力**嵌入教学闭环**（出题 → 练习 → 评测 → 反馈 → 看板），所有数据回流教师端

## 👤 我的角色

- **后端主程**:独立完成 Spring Boot 后端架构设计、数据库设计、核心 API 开发
- **AI 集成**:负责 DeepSeek API 对接、多智能体协作逻辑、Prompt 工程调优
- **核心模块**:AI 智能问答、代码评测调度（Python Judge 子进程）、Sa-Token 鉴权、学生学习数据看板
- **协作分工**:另一位同学负责前端 Vue 3 实现与 UI 交互

## 🧩 技术难点与解决

| 难点 | 解决思路 |
|------|----------|
| 代码评测沙箱隔离 | 独立 Python 评判子进程 + 3 秒超时 + 51200 字符上限，避免死循环与恶意输入 |
| DeepSeek 响应延迟影响交互 | 流式输出 + 前端打字机渲染，首 token 延迟压到 1.5s 内 |
| RAG 检索质量不稳定 | 文档分块策略调优（按段落+标题层级）+ 向量与关键词混合召回 + 重排序 |
| Live2D 与 Vue 集成 | Pixi.js 动态加载模型，组件懒加载避免首屏阻塞 |
| 多角色权限隔离 | Sa-Token 注解 + 自定义拦截器，学生端/教师端路由 + 按钮级双层校验 |

## 📊 效果数据

- 课程试点期间覆盖 **2 个班 / 87 名学生**，累计提问 **3,200+ 次**
- AI 助教单次回答平均耗时 **< 2.3s**，代码评测响应 **< 4s**
- 学生活跃度对比上一学期同课程 **+42%**，作业平均完成时长 **-18%**
- 教师端薄弱知识点识别准确率（与人工标注对比）**82%**
